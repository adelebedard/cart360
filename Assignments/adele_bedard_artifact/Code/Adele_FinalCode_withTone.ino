...
//this array has 89 notes, from index 0 to 88
int turnFactor = 0;
...
void printGyro()
{
  ...
  int gRoundedx = round(imu.calcGyro(imu.gx));
  int gRoundedy = round(imu.calcGyro(imu.gy));
  int gRoundedz = round(imu.calcGyro(imu.gz));
  Serial.print(gRoundedx);
  Serial.print(", ");
  Serial.print(gRoundedy);
  Serial.print(", ");
  Serial.print(gRoundedz);
  Serial.println(" deg/s");
  turnFactor = (int) (((sqrt(sq(gRoundedx) + sq(gRoundedy) + sq(gRoundedz)))*89)/500);
  if (turnFactor > 88 ){
    Serial.println("VALEUR NEGATIVEEEE");
  }
...
{  
  ...
  if ( currentAccel < 0.2 ){
    Serial.println("EN CHUTE LIBREEEEEEEEEEEEEEEEEEEEE");
    Serial.println(currentAccel);
    Serial.println(prevAccel);
    Serial.println(notes[turnFactor]);
    tone(9, notes[turnFactor], 200);
  }    
